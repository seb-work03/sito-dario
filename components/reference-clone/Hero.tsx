  ...
  return (
    <section ref={ref} className="relative bg-[#0D1218] pt-[92px] md:pt-[108px] pb-3 px-5">
      <div className="mx-auto max-w-[1240px] relative">
        ...
        {/* Card + portrait move together so the photo's bottom edge always lines up with the card's bottom border */}
        <motion.div style={{ y: cardY }} className="relative">
          ...
          >
            ...
            <div className="relative grid grid-cols-[1fr_auto_1fr] items-center gap-2 md:gap-4 min-h-[clamp(320px,min(55vw,56vh),521px)] px-5 sm:px-6 md:px-10 py-6 md:py-0">
              ...
              <div className="w-[clamp(252px,38vw,400px)] md:w-[clamp(240px,20vw,520px)] shrink-0" aria-hidden="true" />
          ...
          {/* Portrait — Dario, overflow on card. bottom-0 here is anchored to this wrapper,
             whose height is set by the card, so the photo's bottom always sits exactly on the card's bottom edge. */}
          <motion.div
            initial={{ opacity: 0, y: 30 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 1, delay: 0.15, ease: [0.19, 1, 0.22, 1] }}
            className="absolute z-20 left-[67%] -translate-x-1/2 md:left-1/2 bottom-0 w-[clamp(252px,38vw,400px)] md:w-[clamp(240px,20vw,520px)] aspect-[650/1080] pointer-events-none"
          >
            <Image
              src={PORTRAIT_URL}
